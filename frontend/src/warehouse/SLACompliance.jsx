...
const SLACompliance = () => {
  ...
  const mockData = Array(5).fill(0).map((_,i) => ({metric: 'Sample ' + i, target: 'Sample ' + i, actual: 'Sample ' + i, status: 'Sample ' + i}));
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">SLA Compliance</h2>
...
<th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Metric</th>\n<th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Target</th>\n<th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Actual</th>\n<th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Status</th>
</tr></thead>
      <tbody className="divide-y divide-slate-50">
        {filteredData.map((row, i) => (
          <tr key={i} className="hover:bg-slate-50/50 transition-colors">
<td className="px-4 py-3 text-sm text-slate-600 whitespace-nowrap">{row.metric || 'N/A'}</td>\n<td className="px-4 py-3 text-sm text-slate-600 whitespace-nowrap">{row.target || 'N/A'}</td>\n<td className="px-4 py-3 text-sm text-slate-600 whitespace-nowrap">{row.actual || 'N/A'}</td>\n<td className="px-4 py-3 text-sm text-slate-600 whitespace-nowrap">{row.status || 'N/A'}</td>
          </tr>
        ))}
      </tbody>
    </table>
