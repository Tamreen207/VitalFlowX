...
const HandoffPrep = () => {
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Handoff Preparation</h2>
  ...
  <input type="text" name="shipment_id" placeholder="Enter Shipment ID" required className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-teal-500 focus:outline-none" />
</div>
<div className="space-y-2">
  <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Notes</label>
  <input type="text" name="notes" placeholder="Enter Notes" required className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-teal-500 focus:outline-none" />
</div>
    <button type="submit" disabled={isSubmitting} className={`mt-8 w-full py-3 rounded-xl font-semibold text-white transition-colors ${isSubmitting ? 'bg-slate-400' : 'bg-teal-600 hover:bg-teal-700'}`}>
